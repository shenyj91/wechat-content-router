...
description: >
  把微信里转来的内容链接自动识别并导入 Obsidian或本地普通文件夹。当前优先支持“小红书链接/分享文案”和“微信公众号文章链接”，适用于“从微信 filehelper 抓最近链接并落库”“把一条公众号或小红书内容沉淀进知识库”这类任务。不要用于批量扫描整个微信，也不要用于社交互动。
---

# wechat-content-router

这个 skill 是**微信入口母 skill**。

它不只处理小红书，还处理微信公众号；而且不一定非要进 Obsidian，也可以直接下载到本地文件夹。以后还能继续加别的路由。
