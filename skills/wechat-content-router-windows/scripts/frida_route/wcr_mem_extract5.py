"""
Extract decrypted SQLite databases from WeChat memory (binary transfer).
自动找 Weixin.exe PID；把内存里解密后的数据库整库 dump 为明文 .db，
输出到本目录 output/memdb/，可直接用 sqlite3 打开（含消息/链接）。
前置：pip install frida；微信已启动并登录；建议管理员权限。
仅用于处理你自己的微信数据。
"""
import frida, sys, json, time, struct, sqlite3, os, argparse
...
function parseVarint(arr, pos) {
    let val = 0, shift = 0;
    while (pos < arr.length) { const b = arr[pos++]; val |= (b & 0x7f) << shift; shift += 7; if (!(b & 0x80)) break; }
    return [val, pos];
}
function extractTableNames(pageData) {
    ...
    for (let i = 0; i < Math.min(numCells, 30); i++) {
        ...
        try {
            ...
            const serialTypes = [];
            while (pos < hdrEnd && pos < arr.length) { const [st, p5] = parseVarint(arr, pos); pos = p5; serialTypes.push(st); }
            let dpos = hdrEnd;
            for (let si = 0; si < serialTypes.length; si++) {
                const st = serialTypes[si];
                if (si === 1) {
                    if (st >= 13 && st % 2 === 1) { const textLen = (st - 13) >> 1; let text = ''; for (let t = 0; t < textLen && dpos + t < arr.length; t++) text += String.fromCharCode(arr[dpos + t]); names.push(text); }
                    break;
                }
                if (st === 0) continue;
                if (st <= 4) dpos += st;
    ...
    try {
        ...
        for (const m of matches) {
            try {
                ...
                if (tables.length > 0) {
                    const hasInteresting = tables.some(t => ['msg','session','contact','name2id','chat','message','kv'].some(k => t.toLowerCase().includes(k)));
                    foundDbs.push({ addr: m.address.toString(), dbSize, pageSize: pageSize === 1 ? 65536 : pageSize, reserved, tables, interesting: hasInteresting });
...
for (const db of foundDbs) {
    if (!db.interesting && db.dbSize < 10) continue;
    try {
        const totalBytes = Math.max(db.dbSize, 1) * db.pageSize;
        const readBytes = Math.min(totalBytes, 5 * 1024 * 1024);
        const buf = ptr(db.addr).readByteArray(readBytes);
        send({type: "db_data", addr: db.addr, tables: db.tables, dbSize: db.dbSize, size: readBytes}, buf);
    } catch(e) { send({type: "db_fail", addr: db.addr, error: e.toString()}); }
}
send({type: "all_done"});
"""
...
def on_message(message, data, store):
    if message["type"] == "send":
        ...
        if t == "status":
            ...
            print(f"\n[*] Scan complete. {p['found']} databases found.")
        elif t == "db_data":
            addr = p["addr"]; size = p["size"]
            print(f"  >> Got {size} bytes from {addr} (tables: {p['tables'][:5]})")
            store["data"][addr] = data
        elif t == "db_fail":
            print(f"  >> FAIL {p['addr']}: {p['error']}")
        elif t == "all_done":
...
def main():
    ...
    for addr, data in store["data"].items():
        if not data or len(data) < 100:
            continue
        out_path = os.path.join(MEMDB_DIR, f"mem_{addr.replace('0x','')}.db")
        with open(out_path, "wb") as f:
            f.write(data)
        print(f"\n[{addr}] Saved {len(data)} bytes -> {out_path}")
        try:
