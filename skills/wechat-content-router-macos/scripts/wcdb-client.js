...
class WcdbClient {
  ...
  _findDllDir() {
    ...
    const platformDir = platform === 'win32' ? 'win32' : (platform === 'darwin' ? 'macos' : 'linux')
    const archDir = platform === 'darwin' ? 'universal' : (process.arch === 'arm64' ? 'arm64' : 'x64')
    const libName = platform === 'win32' ? 'wcdb_api.dll'
      : (platform === 'darwin' ? 'libwcdb_api.dylib' : 'libwcdb_api.so')

    for (const root of roots) {
      const candidate = join(root, 'wcdb', platformDir, archDir)
      if (existsSync(join(candidate, libName))) return candidate
      // Windows 仅有 x64 兜底
      if (platform === 'win32') {
        const fallback = join(root, 'wcdb', platformDir, 'x64')
        if (existsSync(join(fallback, libName))) return fallback
      }
    }
    return null
  }
  ...
  async initialize() {
    ...
    if (!dllDir) throw new Error('wcdb_api 原生库未找到。请确认 resources/wcdb/<平台> 下存在对应原生库，或从 WxLens 项目根目录运行。')
    ...
    if (process.platform === 'win32') {
      ...
      // macOS：libwcdb_api.dylib 自带 @loader_path rpath，会自动拉起同目录的
      // WCDB.framework，无需手动预加载核心库（避免框架自引用解析失败）。
      const dllPath = join(dllDir, 'libwcdb_api.dylib')
      this.lib = this.koffi.load(dllPath)
    } else {
      // Linux
      const wcdbCorePath = join(dllDir, 'libWCDB.so')
      if (existsSync(wcdbCorePath)) try { this.koffi.load(wcdbCorePath) } catch {}
      const dllPath = join(dllDir, 'libwcdb_api.so')
      this.lib = this.koffi.load(dllPath)
    }
